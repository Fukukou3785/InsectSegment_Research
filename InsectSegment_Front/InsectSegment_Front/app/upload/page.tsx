...
// ...
  ...
  // page.tsx 460行目あたり
  const handleProcess = async () => { // asyncを追加
    if (selectedImage) {
      ...
      try {
        // 2. FastAPIサーバーに画像をPOST
        console.log("Sending request to API..."); // ★デバッグ用ログを追加
        const response = await fetch('http://127.0.0.1:8000/api/segment', {
          ...
          body: JSON.stringify({
            image_base64: imageToSegment // Base64文字列を送信
          }),
        });
        console.log("API response received:", response.status); // ★デバッグ用ログを追加
        ...
        // 3. サーバーから返ってきた「色付け済み」画像(Base64)を受け取る
        const result: { segmented_image_base64: string } = await response.json();

        // 4. 元の画像と「セグメント結果」の両方をsessionStorageに保存
        sessionStorage.setItem("insectImage", selectedImage); // 元画像
        sessionStorage.setItem("segmentedImage", result.segmented_image_base64); // セグメント結果

        // 5. processingページに遷移
        router.push("/processing");

      } catch (error) {
        console.error("Error calling segment API:", error); // ★エラーをコンソールに出力
        alert(error instanceof Error ? error.message : 'APIの呼び出しに失敗しました。');
        setIsProcessing(false); // エラー時はローディング解除
      }
    }
  }
